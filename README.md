...
This repository contains instructions for participation in the [shared task on word sense induction and disambiguation for the Russian language](http://russe.nlpub.org/2018/wsi). **TLDR**: You are given a word, e.g. ```замок``` and a bunch of text fragments (contexts) where this word appears, e.g. ```замок владимира мономаха в любече``` and  ```передвижению засова ключом замка, пока пластина```. You need to cluster these contexts in the (unknown in advance) number of groups which correspond to v0arious senses of the word. In this example you want to have two groups with the contexts of the "lock" and the "castle" senses of the word ```замок```. 
